...
class HoneyPotBaseProtocol(insults.TerminalProtocol, TimeoutMixin):
    ...
    def switch_user(self, username, uid, gid, home):
        """Switch the effective user identity for this session."""
        self.user.uid = uid
        self.user.gid = gid
        self.user.username = username
        self.user.avatar.uid = uid
        self.user.avatar.gid = gid
        self.user.avatar.username = username
        self.user.avatar.home = home
        self.environ["USER"] = username
        self.environ["LOGNAME"] = username
        self.environ["HOME"] = home
        self.environ["UID"] = str(uid)
        if uid == 0:
    ...
    def getCommand(self, cmd, paths):
        ...
        # Check config-specified txtcmds_path first (profile-generated outputs),
        # then fall back to the built-in package txtcmds.
        txtcmds_config_path = CowrieConfig.get(
            "honeypot", "txtcmds_path", fallback=""
        )
        if txtcmds_config_path:
            import pathlib
            config_txtcmd = pathlib.Path(txtcmds_config_path) / path.lstrip("/")
            if config_txtcmd.is_file():
                try:
                    binary_data = config_txtcmd.read_bytes()
    ...
    def eofReceived(self) -> None:
        ...
        """
        # In HoneyNet mode, ignore EOF for interactive shells. The upstream
        # proxy (paramiko) may send EOF as part of channel setup, but the
        # session should stay alive until the SSH transport disconnects.
        if os.environ.get("HONEYNET_MODE") == "true":
            log.msg("eofReceived: ignoring EOF in HONEYNET_MODE")
            return
...
class HoneyPotExecProtocol(HoneyPotBaseProtocol):
    ...
    input_data = b""
    # When True, an LLM deferred is in-flight; defer eofReceived close.
    llm_pending = False
    ...
    def connectionMade(self) -> None:
        ...
        # Initialize hybrid LLM fallback handler for exec sessions too
        if hybrid_llm_enabled:
            from cowrie.shell.llm_fallback import LLMFallbackHandler
            self.llm_fallback_handler = LLMFallbackHandler(self)
    ...
    def eofReceived(self) -> None:
        """Defer session close if an LLM request is still in-flight."""
        if self.llm_pending:
            return
        super().eofReceived()
...
class HoneyPotInteractiveProtocol(HoneyPotBaseProtocol, recvline.HistoricRecvLine):
    ...
    def connectionMade(self) -> None:
        ...
        # Initialize hybrid LLM fallback handler for this session
        if hybrid_llm_enabled:
            from cowrie.shell.llm_fallback import LLMFallbackHandler
            self.llm_fallback_handler = LLMFallbackHandler(self)
    ...
    def _active_proxy(self):
        """Return the active SSH proxy on the cmdstack, or None."""
        if self.cmdstack:
            cmd = self.cmdstack[-1]
            if hasattr(cmd, 'proxy') and cmd.proxy.active:
                return cmd.proxy
        return None
    ...
    def characterReceived(self, ch, moreCharactersComing):
        ...
        # When an SSH proxy is active, relay raw bytes to the remote PTY
        # and let it handle echo. This prevents double-echo on nested hops.
        proxy = self._active_proxy()
        if proxy:
            proxy.relay_input_bytes(ch)
            return
