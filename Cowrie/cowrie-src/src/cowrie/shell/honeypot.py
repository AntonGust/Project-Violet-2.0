...
# Hybrid LLM fallback: conditionally import when enabled
hybrid_llm_enabled = CowrieConfig.getboolean("hybrid_llm", "enabled", fallback=False)
if hybrid_llm_enabled:
    from cowrie.shell.llm_fallback import LLMFallbackHandler
...
class HoneyPotShell:
    def __init__(
        ...
        # Line continuation state for heredocs and unclosed quotes
        self._line_buffer: str = ""
        self._heredoc_delim: str | None = None
        self._heredoc_lines: list[str] = []
        self._heredoc_redirect: str | None = None
        self._heredoc_cmd: str | None = None

        # this is the first prompt after starting
        self.showPrompt()

    def lineReceived(self, line: str) -> None:
        # --- Heredoc buffering mode ---
        if self._heredoc_delim is not None:
            if line.strip() == self._heredoc_delim:
                self._finish_heredoc()
            else:
                self._heredoc_lines.append(line)
                self.protocol.terminal.write(b"> ")
                self.protocol.ps = (self.protocol.ps[0], b"> ")
            return

        # --- Quote continuation mode ---
        ...
        while True:
            try:
                ...
                # shlex raises ValueError on unclosed quotes — enter continuation mode
        ...
        # --- Heredoc detection: scan tokens for << before executing ---
        if self.cmdpending:
            last_cmd = self.cmdpending[-1]
            heredoc_idx = None
            for i, t in enumerate(last_cmd):
                if t == "<<":
                    heredoc_idx = i
                    break

            if heredoc_idx is not None and heredoc_idx + 1 < len(last_cmd):
                delim = last_cmd[heredoc_idx + 1].strip("'\"")
                # Find redirect target (> file) in the tokens before <<
                redirect_target = None
                cmd_tokens = last_cmd[:heredoc_idx]
                for j, t in enumerate(cmd_tokens):
                    if t == ">" and j + 1 < len(cmd_tokens):
                        redirect_target = cmd_tokens[j + 1]
                        cmd_tokens = cmd_tokens[:j]
                        break

                self._heredoc_delim = delim
                self._heredoc_redirect = redirect_target
                self._heredoc_cmd = " ".join(cmd_tokens) if cmd_tokens else None
                self._heredoc_lines = []
                # Remove the incomplete command from pending
                self.cmdpending.pop()
                self.protocol.terminal.write(b"> ")
                self.protocol.ps = (self.protocol.ps[0], b"> ")
                return
    ...
    def _finish_heredoc(self) -> None:
        """Assemble heredoc body and execute the associated command or redirect."""
        body = "\n".join(self._heredoc_lines)
        redirect_target = self._heredoc_redirect
        cmd = self._heredoc_cmd

        # Reset heredoc state
        self._heredoc_delim = None
        self._heredoc_lines = []
        self._heredoc_redirect = None
        self._heredoc_cmd = None

        if redirect_target:
            # Write body to file (e.g. cat << EOF > /tmp/file)
            outfile = self.protocol.fs.resolve_path(
                redirect_target, self.protocol.cwd
            )
            try:
                ...
                pass
            # Write the actual content to honeyfs so cat can read it back
            real_file = self.protocol.fs.realfile(
                self.protocol.fs.getfile(outfile), outfile
            )
            if real_file:
                try:
            ...
            # Feed body as stdin to the command (e.g. cat << EOF)
            self.protocol.terminal.write((body + "\n").encode("utf8"))
    ...
    def runCommand(self):
        ...
        for index, cmd in reversed(list(enumerate(cmd_array))):
            ...
            if cmdclass:
                ...
                cmd_string = cmd["command"] + " " + " ".join(cmd["rargs"])
                ...
                if hybrid_llm_enabled and hasattr(self.protocol, "llm_fallback_handler"):
                    # Forward unknown command to LLM fallback
                    from twisted.internet import defer

                    # Prevent eofReceived from closing the session while
                    # the LLM request is in-flight.
                    self.protocol.llm_pending = True

                    handler = self.protocol.llm_fallback_handler
                    d = handler.handle_command(cmd_string)
                    d.addCallback(self._write_llm_response)
                    d.addErrback(self._llm_error_fallback, cmd["command"])
                    pp = None
                    break
                else:
    ...
    def _write_llm_response(self, response: str) -> None:
        ...
        # Continue processing remaining semicolon-separated commands
        if self.cmdpending:
            self.runCommand()
        else:
            self._finishOrPrompt()

    def _llm_error_fallback(self, failure_obj: Any, cmd: str) -> None:
        """On LLM error, fall back to 'command not found', then continue pending commands."""
        self.protocol.llm_pending = False
        log.msg(f"HybridLLM fallback error: {failure_obj}")
        message = f"-bash: {cmd}: command not found\n".encode("utf8")
        self.protocol.terminal.write(message)
        # Continue processing remaining semicolon-separated commands
        if self.cmdpending:
            self.runCommand()
    ...
    def _finishOrPrompt(self) -> None:
        """For exec sessions, signal completion so the SSH channel flushes output.
        For interactive sessions, show the prompt."""
        from cowrie.shell import protocol

        if isinstance(self.protocol, protocol.HoneyPotExecProtocol):
            # Guard: the connection may already be gone if the LLM response
            # arrived after the SSH client disconnected.
            terminal = getattr(self.protocol, "terminal", None)
            transport = getattr(terminal, "transport", None) if terminal else None
            if transport is not None:
                exit_status = failure.Failure(error.ProcessDone(status=""))
                transport.processEnded(exit_status)
        else:
            self.showPrompt()
