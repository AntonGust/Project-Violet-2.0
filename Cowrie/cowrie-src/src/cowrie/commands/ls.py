...
class Command_ls(HoneyPotCommand):
    ...
    def do_ls_l(self, path: str) -> None:
        ...
        # Print "total N" line (sum of 1K blocks, as GNU ls does)
        total_blocks = 0
        for file in files:
            if file[fs.A_NAME].startswith(".") and not self.showHidden:
                continue
            # Each file uses at least 4K (1 block of 4096), dirs use 4K
            total_blocks += max(4, (file[fs.A_SIZE] + 1023) // 1024)
        self.write(f"total {total_blocks}\n")
