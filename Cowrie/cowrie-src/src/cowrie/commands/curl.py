...
def _is_internal_url(hostname: str) -> bool:
    """Check if a hostname resolves to an internal/private/link-local address.

    Returns True for:
      - RFC1918 private ranges (10.x, 172.16-31.x, 192.168.x)
      - Link-local (169.254.x.x) — includes EC2 metadata endpoint
      - Loopback (127.x.x.x)
      - Hostnames that look internal (contain .internal, .corp, .local, etc.)
      - Unix socket paths (--unix-socket)
    """
    if not hostname:
        return False

    # Check for internal-looking hostnames
    internal_suffixes = (
        ".internal", ".corp", ".local", ".lan", ".intranet",
        ".private", ".home", ".localdomain",
    )
    hl = hostname.lower()
    if any(hl.endswith(s) for s in internal_suffixes):
        return True
    if hl in ("localhost",):
        return True
    ...
    # Try DNS resolution (non-blocking best-effort)
    try:
        resolved = socket.getaddrinfo(hostname, None, socket.AF_INET, socket.SOCK_STREAM)
        ...
        # Can't resolve — if it has an internal-looking name, treat as internal
        if "." not in hostname or hostname.count(".") == 0:
            return True  # bare hostname like "jenkins-ci" or "db-replica-01"
...
class Command_curl(HoneyPotCommand):
    ...
    def start(self):
        try:
            optlist, args = getopt.getopt(
                self.args, "kLsho:IOu:A:e:H:X:d:m:", ["help", "manual", "silent", "head", "insecure",
                "compressed", "connect-timeout=", "header=", "request=", "data=",
                "user=", "user-agent=", "referer=", "max-time=", "retry=",
                "retry-delay=", "retry-max-time=", "cacert=", "cert=",
                "key=", "location", "fail", "show-error", "verbose",
                "no-keepalive", "no-buffer", "no-sessionid", "tcp-nodelay",
                "unix-socket=", "write-out=", "proxy=",
                ]
            )
        ...
        # Check for --unix-socket (Docker API access etc.) — always route to LLM
        raw_cmd = " ".join(self.args) if self.args else ""
        has_unix_socket = "--unix-socket" in raw_cmd

        # Sandbox: route internal/private URLs to LLM fallback instead of real HTTP
        if has_unix_socket or _is_internal_url(self.host):
            log.msg(f"curl: internal URL detected ({self.host}), routing to LLM fallback")
            yield self._route_to_llm_fallback(url)
            return
    ...
    @inlineCallbacks
    def _route_to_llm_fallback(self, url: str) -> None:
        """Route internal URLs to the LLM fallback handler for realistic fake responses."""
        handler = getattr(self.protocol, "llm_fallback_handler", None)
        if handler is None:
            # No LLM fallback available — return a connection refused error
            self.errorWrite(
        ...
        # Build the full curl command string for LLM context
        cmd_str = "curl " + " ".join(self.args) if self.args else f"curl {url}"
        try:
            self.protocol.llm_pending = True
            response = yield handler.handle_command(cmd_str)
            if response:
                self.write(response)
                if not response.endswith("\n"):
                    self.write("\n")
        except Exception as e:
            log.msg(f"curl LLM fallback error: {e}")
            self.errorWrite(
                f"curl: (7) Failed to connect to {self.host} port {self.port}: Connection refused\n"
            )
        finally:
