...
class Command_find(HoneyPotCommand):
    ...
    def start(self) -> None:
        ...
        while idx < len(self.args):
            ...
            if arg == "-name":
                ...
                # Unsupported flag — delegate to LLM fallback if available
                handler = getattr(self.protocol, "llm_fallback_handler", None)
                if hybrid_llm_enabled and handler:
                    from twisted.internet import defer

                    cmd_string = "find " + " ".join(self.args)
                    d = handler.handle_command(cmd_string)
                    d.addCallback(self._write_llm_and_exit)
                    d.addErrback(self._llm_error_and_exit, arg)
                    return
                else:
    ...
    def _write_llm_and_exit(self, response: str) -> None:
        if response:
    ...
    def _llm_error_and_exit(self, failure_obj, arg: str) -> None:
        self.errorWrite(f"find: unknown argument '{arg}'\n")
        self.exit()
