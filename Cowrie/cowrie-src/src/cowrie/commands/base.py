...
class Command_id(HoneyPotCommand):
    # Standard GID mappings for common groups
    _KNOWN_GIDS: dict[str, int] = {
        "root": 0, "daemon": 1, "bin": 2, "sys": 3, "adm": 4,
        "tty": 5, "disk": 6, "lp": 7, "mail": 8, "sudo": 27,
        "www-data": 33, "backup": 34, "shadow": 42, "staff": 50,
        "users": 100, "docker": 999, "plugdev": 46,
    }
                ...
                if pu["name"] == username:
                    ...
                    if profile_groups:
                        ...
                        for g in profile_groups:
                            if g == username:
                                parts.append(f"{u.gid}({g})")
                            elif g in self._KNOWN_GIDS:
                                parts.append(f"{self._KNOWN_GIDS[g]}({g})")
                            else:
                                parts.append(f"{u.gid + 1}({g})")
