...
class Command_head(HoneyPotCommand):
    ...
    def start(self) -> None:
        ...
        if not self.args or self.args[0] == ">":
            ...
            # Support traditional -NUMBER syntax (e.g., head -40 file)
            # by converting it to -n NUMBER before getopt parsing.
            processed_args = []
            for arg in self.args:
                if re.match(r"^-\d+$", arg):
                    processed_args.extend(["-n", arg[1:]])
                else:
                    processed_args.append(arg)

            try:
                optlist, args = getopt.getopt(processed_args, "c:n:qv")
            except getopt.GetoptError as err:
