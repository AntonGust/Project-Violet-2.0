...
Destination     Gateway         Genmask         Flags   MSS Window  irtt Iface\n"""
    ...
    def _get_profile_services(self) -> list[dict[str, Any]]:
        """Get services from the loaded profile, if available."""
    ...
    def _format_listen_line(self, port: int, pid: int | None, program: str | None) -> str:
        """Format a single LISTEN line for a given port."""
        if self.show_numeric:
            port_str = str(port)
        else:
            port_str = _PORT_NAMES.get(port, str(port))

        local = f"0.0.0.0:{port_str}"
        foreign = "0.0.0.0:*"
        local_padded = f"{local:<24}"
        foreign_padded = f"{foreign:<24}"

        if self.show_programs and pid and program:
            prog_str = f"{pid}/{program}"
            return f"tcp        0      0 {local_padded}{foreign_padded}LISTEN      {prog_str}\n"
        elif self.show_programs:
            return f"tcp        0      0 {local_padded}{foreign_padded}LISTEN      -\n"
        else:
            return f"tcp        0      0 {local_padded}{foreign_padded}LISTEN\n"

    def _format_listen6_line(self, port: int, pid: int | None, program: str | None) -> str:
        """Format a single IPv6 LISTEN line for a given port."""
        if self.show_numeric:
            port_str = str(port)
        else:
            port_str = _PORT_NAMES.get(port, str(port))
        ...
        if self.show_programs and pid and program:
            prog_str = f"{pid}/{program}"
            return f"tcp6       0      0 {local_padded}{foreign_padded}LISTEN      {prog_str}\n"
        elif self.show_programs:
            return f"tcp6       0      0 {local_padded}{foreign_padded}LISTEN      -\n"
        else:
            return f"tcp6       0      0 {local_padded}{foreign_padded}LISTEN\n"
...
Proto Recv-Q Send-Q Local Address           Foreign Address         State\n"""
        ...
        # Build LISTEN lines from profile services
        profile_services = self._get_profile_services()
        listen_entries: list[tuple[int, int | None, str | None]] = []

        if profile_services:
            for svc in profile_services:
                for port in svc.get("ports", []):
                    # Extract program name from command (basename)
                    cmd = svc.get("command", "")
                    prog = cmd.split("/")[-1].split()[0] if cmd else svc.get("name", "")
                    listen_entries.append((port, svc.get("pid"), prog))
        else:
            # Fallback: just SSH
            listen_entries.append((22, None, "sshd"))

        if self.show_listen or self.show_all:
            for port, pid, prog in listen_entries:
                self.write(self._format_listen_line(port, pid, prog))
        ...
        if self.show_listen or self.show_all:
            for port, pid, prog in listen_entries:
                self.write(self._format_listen6_line(port, pid, prog))
        self.write(
...
unix  3      [ ]         STREAM     CONNECTED     8619     @/com/ubuntu/upstart\n"""
    ...
    def call(self) -> None:
        ...
        for x in self.args:
            ...
            if x.startswith("-") and x.count("p"):
                self.show_programs = True
