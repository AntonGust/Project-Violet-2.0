...
class CowrieSSHChannel(channel.SSHChannel):
    ...
    def closeReceived(self) -> None:
        """
        Called when the remote side sends SSH_MSG_CHANNEL_CLOSE.

        In HONEYNET_MODE, ignore the remote close to keep the session alive.
        Hop1's paramiko sends EOF + CHANNEL_CLOSE as part of channel setup,
        but hop2's session must stay alive for the SSH proxy to connect onward.
        The session will be torn down when the SSH transport itself disconnects.
        """
        if os.environ.get("HONEYNET_MODE") == "true":
            log.msg("closeReceived: ignoring remote close in HONEYNET_MODE")
            return
