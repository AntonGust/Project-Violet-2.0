...
# NOTE: This script is a legacy Blue_Lagoon-era TextGrad experiment.
# _legacy_finalize and _legacy_save were removed in the
# Phase 4 profile-based rewrite.  The functions below are stubbed out so
# the module can still be imported without crashing, but this script
# should not be used in production.

def _legacy_finalize(config):
    """Stub replacing the removed _legacy_finalize."""
    import uuid
...
def _legacy_save(config, path=None):
    """Stub replacing the removed _legacy_save."""
    import json
    from pathlib import Path
    output_dir = Path(__file__).resolve().parent.parent / "Blue_Lagoon" / "DefaultConfigs"
    output_dir.mkdir(parents=True, exist_ok=True)
    config_id = config.get("id", "unknown")
    filepath = output_dir / f"config_{config_id}.json"
...
def main():
    ...
    clean_config = _legacy_finalize(config)

    print(json.dumps(clean_config, indent=4))
    _legacy_save(clean_config)
