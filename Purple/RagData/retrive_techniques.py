...
def retrieve_unique_techniques(platforms=None):
    ...
    unique_techniques = []
    for obj in mitre_attack_data['objects']:
        if obj['type'] != 'attack-pattern':
            continue
        if obj.get('x_mitre_deprecated', False) or obj.get('revoked', False):
            continue
        if platforms:
            obj_platforms = obj.get('x_mitre_platforms', [])
            if not any(p in obj_platforms for p in platforms):
                continue
        unique_techniques.append({'id': obj['external_references'][0]['external_id'], 'name': obj['name']})
